...
class Window(QMainWindow):
	...
	def __init__(self,name,client,window_type,app,parent=None):
		...
		if self.window_type==CHANNEL_WINDOW:
			...
			# Create the spacer for between the topic editor
			# and the user count
	...
	def areAllTypesFiltered(self):
		if config.SHOW_CHANNEL_JOIN_MESSAGES: return False
		if config.SHOW_CHANNEL_PART_MESSAGES: return False
		if config.SHOW_CHANNEL_QUIT_MESSAGES: return False
		if config.SHOW_CHANNEL_MODE_CHANGE_MESSAGES: return False
		if config.SHOW_CHANNEL_NICK_MESSAGES: return False
		if config.SHOW_CHANNEL_TOPIC_MESSAGES: return False
		return True
		...
		try:
			if type(message)==type(str()):
				...
				if message.sender!='':
					...
					if self.is_ignored(nickname,hostmask):
...
class Highlighter(QSyntaxHighlighter):
	...
	def highlightBlock(self, text):
		...
		if config.APPLY_SYNTAX_STYLES_TO_INPUT_WIDGET:
			...
			# First, check to make sure that a full hostID or is
					...
					for word_object in re.finditer(self.HOSTIDS, text):
						for name in self.parent.parent.getAllHostids():
							if name == word_object.group():
								do_not_spellcheck.append(name)
								self.setFormat(word_object.start(), word_object.end() - word_object.start(), channelformat)
					# Now we highlight plain old hosts
					for word_object in re.finditer(self.HOSTS, text):
						for name in self.parent.parent.getAllHosts():
							if name == word_object.group():
								do_not_spellcheck.append(name)
								self.setFormat(word_object.start(), word_object.end() - word_object.start(), channelformat)
