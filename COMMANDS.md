...
| `/clear [SERVER] [WINDOW]`              | Clears a subwindow's chat display. `SERVER` is optional if `WINDOW` belongs to the same context. Pass `*` as `WINDOW` to clear the server subwindow  |
| `/close [SERVER] [WINDOW]`              | Closes a subwindow. `SERVER` is optional if `WINDOW` belongs to the same context. Pass `*` as `WINDOW` to hide the server subwindow                    |
...
| `/error [SERVER] [WINDOW] TEXT...`     | Prints an error message to a subwindow, and immediately exits a script if called from a script. `SERVER` is optional if `WINDOW` belongs to the same context. Pass `*` as `SERVER` to print to all open subwindows. Pass `*` as `WINDOW` to print to the server subwindow     |
...
| `/fade [SERVER] [WINDOW] PERCENTAGE`    | Sets transparency of a subwindow by `PERCENTAGE`. `SERVER` is optional if `WINDOW` belongs to the same context. Call without arguments to see current subwindow's transparency. Pass `*` as `WINDOW` to set the transparency of the server subwindow     |
...
| `/hide [SERVER] [WINDOW]`                                 | Hides a subwindow. `SERVER` is optional if `WINDOW` belongs to the same context. Pass `*` as `WINDOW` to hide the server subwindow      |
...
| `/maximize [SERVER] [WINDOW]`             | Maximizes a subwindow. `SERVER` is optional if `WINDOW` belongs to the same context. Pass `*` as `WINDOW` to maximize the server subwindow         |
| `/me MESSAGE...`                        | Sends a CTCP action message to the current chat                                                                                  |
| `/minimize [SERVER] [WINDOW]`             | Minimizes a subwindow. `SERVER` is optional if `WINDOW` belongs to the same context. Pass `*` as `WINDOW` to minimize the server subwindow                    |
| `/mode TARGET MODE...`                  | Sets a mode on a channel or user                                                                                                 |
| `/move [SERVER] [WINDOW] X Y` | Moves a subwindow to `X` (left and right) and `Y` (up and down) coordinates. `SERVER` is optional if `WINDOW` belongs to the same context. Call without arguments to see the current subwindow's coordinates. Pass `*` as `WINDOW` to move the server subwindow      |
...
| `/rerender [SERVER] [WINDOW]`              | Re-renders the chat log of a subwindow. Pass `*` as the only argument to re-render all open chat subwindows          |
| `restrict SERVER`\|`CHANNEL`\|`PRIVATE`     | Prevents a script from running if it is not being ran in a `SERVER`, `CHANNEL`, or `PRIVATE` subwindow. Up to two subwindow types can be passed. *Can only be called from scripts*                                                                                |
| `/restore [SERVER] [WINDOW]`              | Restores a subwindow. `SERVER` is optional if `WINDOW` belongs to the same context. Pass `*` as `WINDOW` to restore the server subwindow                                 |
...
| `/window [COMMAND] [X] [Y]`     | Manipulates the main application window. Valid commands are `cascade`, `fade`, `fullscreen`, `hotkey`,  `install`, `layout`, `logs`, `maximize`, `minimize`, `move`, `next`, `ontop`, `pause`, `plugin`, `previous`, `readme`, `restart`, `restore`, `settings`, `size`, `tile`, and `uninstall`. Call with no arguments to see main window information and a list of subwindows                                                 |
...
Tags can be nested, so `<5,9***~Hello~ __world!__***>` is a completely valid statement in **MERK** "markdown". The client can be configured to strip colors and formatting from display, but messages sent in "markdown" will still appear with the formatting in clients the message is sent to. In channel subwindows, topics are automatically changed into "markdown" when the topic is edited via the GUI.
